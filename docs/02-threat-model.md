...
| **T5 — Truncation / partial write** | Length-prefixed framing with declared lengths validated against actual bytes before any allocation. | `TamperDetectionTest` covers the framing. Staged publish **is** implemented in `DriveTransport.upload` (temp object → read-back → archive → re-check generation → rename) but has **never been executed**; see the production-readiness review for the non-atomic window it still leaves between deleting the live file and renaming the temp one. |
